...
const DOCUMENTS_APP_DIR = resolve(__dirname, '../src/Documents/')
...
module.exports = (env) => {
	...
	return {
		entry: './src/Documents/index.tsx',
		resolve: {
			extensions: ['.ts', '.tsx', '.js'],
			modules: [DOCUMENTS_APP_DIR, SHARED_DIR, '../node_modules'],
		},
		output: {
			filename: '[name].[hash].js',
			path: join(__dirname, '../build/documents'),
		},
		module: {
			rules: [{
					test: /\.tsx?$/,
		...
		plugins: [
			...
			new webpack.DefinePlugin({
				'process.env': {
					...
					REACT_APP_DOCUMENTS_URL: JSON.stringify(config.REACT_APP_DOCUMENTS_URL),
