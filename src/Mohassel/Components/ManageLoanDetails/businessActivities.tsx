...
import { BusinessSector, manageLoanDetailsArray } from '../ManageLoanDetails/manageLoanDetailsInitials';
import Swal from 'sweetalert2';
import { getErrorMessage } from '../../../Shared/Services/utils';
import { CRUDList, CrudOption } from '../CRUDList/crudList';
import { createBusinessActivity, editBusinessActivity, getBusinessSectors } from '../../Services/APIs/configApis/config';
...
class BusinessActivities extends Component<{}, State> {
    ...
    async newBusinessActivity(name) {
        this.setState({ loading: true })
        const res = await createBusinessActivity({ BusinessActivityName: name, BusinessSectorId: this.state.sector.id });
        if (res.status === "success") {
            this.setState({
                loading: false,
            }, async () => { await this.getBusinessSectors(); await this.prepareActivites(this.state.sector.id) })
