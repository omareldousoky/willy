...
class EarlyPaymentPDF extends Component<Props, State> {
  ...
  UNSAFE_componentWillMount() {
    ...
    this.props.data.installmentsObject.installments.forEach((installment) => {
      if (
        ...
        latePrincipal +=
          installment.principalInstallment - installment.principalPaid
  ...
  getInstallmentsRemaining() {
    ...
    this.props.data.installmentsObject.installments.forEach((installment) => {
      if (this.state.installmentsDue.includes(installment.id)) {
        total += installment.installmentResponse
      }
    })
    return total
  }
  ...
  getApplicationFee() {
    ...
    if (this.props.data.product.applicationFeePercentPerPerson !== 0) {
      return numbersToArabic(
        this.props.data.product.applicationFeePercentPerPerson
      )
    }
    if (this.props.data.product.individualApplicationFee !== 0) {
      return numbersToArabic(this.props.data.product.individualApplicationFee)
    }
    if (this.props.data.product.applicationFee !== 0) {
      return numbersToArabic(this.props.data.product.applicationFee)
    }
    return numbersToArabic(0)
  }

  render() {
    return (
      <div
        className="early-payment-print"
        style={{ direction: 'rtl' }}
        lang="ar"
      >
        <table
              ...
              <td>
                ...
                <div className="frame">
                  {this.props.data.product.beneficiaryType === 'individual'
                    ? numbersToArabic(this.props.data.customer.key)
                    : numbersToArabic(
          ...
          <tbody>
            <tr>
              ...
              <td>
                ...
                <div className="frame">
                  {this.props.data.product.beneficiaryType === 'individual'
                    ? this.props.data.customer.businessActivity
                    : 'تمويل رأس المال'}
                </div>
              </td>
            </tr>
        ...
        <table className="tablestyle">
          <tbody>
              ...
              (installment, index) => {
                return (
                  <tr key={index}>
                    ...
                    <td
                      className={
                        this.state.installmentsDue.includes(installment.id)
                          ? 'due'
                          : ''
                      }
                    >
            ...
            <tr>
              ...
              <td className="border">
                {numbersToArabic(this.getSum('principalPaid'))}
              </td>
              <td className="border">
                {numbersToArabic(this.getSum('feesPaid'))}
              </td>
              <th className="border">ايام التأخير</th>
              <td className="border">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td className="border">
                {numbersToArabic(
                  this.props.data.installmentsObject.totalInstallments.feesSum -
                    this.getSum('feesPaid') +
                    this.props.earlyPaymentData.remainingPrincipal
                )}
              </td>
              <td className="border">
                {numbersToArabic(
              ...
              <td className="border">
                {numbersToArabic(
                  this.props.data.installmentsObject.totalInstallments.feesSum -
                    ...
                    this.props.earlyPaymentData.remainingPrincipal -
                    (this.props.earlyPaymentData.remainingPrincipal +
                      (this.props.data.product.earlyPaymentFees *
                        this.props.earlyPaymentData.remainingPrincipal) /
                        100)
                )}
              </td>
            ...
            <tr>
              ...
              <th className="border">
                {numbersToArabic(this.props.data.product.earlyPaymentFees)}%
                (تكلفه تمويل الترحيل)
              </th>
