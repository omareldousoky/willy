...
const OfficerBranchPercentPayment = (props: OfficerPercentPaymentProps) => {
    ...
    return (
        <div className="officers-payment officers-branch-payment" lang="ar">
            ...
            >
                <p style={{ margin: "10px auto", fontSize: "16px" }}>
                    ...
                    {data.branches &&
                        data.branches.map((branch) => (
                            <tr key={branch.branch}>
                                ...
                                <td colSpan={2}>
                                    {branch.expectedPayments || "0.00"}
                                </td>
                                <td colSpan={2}>{branch.paid || "0.00"}</td>
                                <td>
                                    {formatPercent(branch.paidPercentage) ||
                                        "%00.00"}
                                </td>
                                <td>{branch.walletActiveLoans || "0"}</td>
                                <td>
                                    {formatPercent(
                                        branch.walletActivePercent
                                    ) || "%00.00"}
                                </td>
                                <td>{branch.walletAmount || "0.00"}</td>
                                <td>
                                    {formatPercent(
                                        branch.walletAmountPercent
                                    ) || "%00.00"}
                                </td>
                                <td colSpan={2}>
                                    {branch.periodCollections || "0.00"}
                                </td>
                            </tr>
                        ))}
                    <tr>
                        ...
                        <td>
                            {formatPercent(data.paidPercentage) || "%00.00"}
                        </td>
