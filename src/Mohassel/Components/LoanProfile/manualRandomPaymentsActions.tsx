...
const ManualRandomPaymentsActions = (props) => {
    return <> {props.pendingActions.map((pendingAction, index) => {
        return (
            <div key={index} className="warning-container">
                ...
                <div className="info">
                    ...
                    <span> {getRandomPaymentByKey(pendingAction.transactions[0].action)}</span>
                </div>
                <div className="info">
                    <span className="text-muted">{local.truthDate}</span>
                    <span>{timeToDateyyymmdd(pendingAction.transactions[0].truthDate)}</span>
                </div>
                <div className="info">
                    <span className="text-muted">{local.dueDate}</span>
                    <span>{timeToDateyyymmdd(pendingAction.transactions[0].actualDate)}</span>
                </div>
                <div className="info">
                ...
                <Can I='payInstallment' a='application'>
                    <div style={{ color: '#000', cursor: 'pointer' }} data-qc="editManualPayment" onClick={() => props.editManualPayment(pendingAction._id)}><span className="fa fa-pencil" style={{ marginLeft: 5 }}></span>{local.edit}</div>
                </Can>
                <Can I='payInstallment' a='application'>
                    <div className="cancel" data-qc="rejectManualPayment" onClick={() => { props.rejectManualPayment(pendingAction._id) }}>{local.cancel}</div>
                </Can>
                <Can I='approvePendingAction' a='application'>
                    <div className="submit" data-qc="approveManualPayment" onClick={() => { props.approveManualPayment(pendingAction._id) }}>{local.submit}</div>
                </Can>
            </div>
        )
    })}</>
