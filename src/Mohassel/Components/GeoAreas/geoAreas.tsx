...
class GeoAreas extends Component<{}, State> {
    ...
    async newGeoArea(name, active) {
        ...
        const res = await addGeoArea({ name: name, branchId: this.state.branch._id, active: active });
    ...
    async editGeoArea(id, name, active) {
        this.setState({ loading: true })
        const res = await updateGeoArea(id, { name: name, active: active, branchId: this.state.branch._id });
        if (res.status === "success") {
            this.setState({
                loading: false,
            }, () => this.getBranchAreas())
    ...
    async getBranchAreas() {
        ...
        if (branchAreas.status === 'success') {
            const areas = (branchAreas.body.data) ? branchAreas.body.data.map(area => ({ name: area.name, id: area._id, activated: area.active, disabledUi: true })) : [];
            this.setState({
