...
export const loanCreationValidation = Yup.object().shape({
  loanCreationDate: Yup.string()
    .test(
      'Should not be before acceptance date',
      local.creationDateCannotBeBeforeAcceptanceDate,
...
export const loanIssuanceValidation = Yup.object().shape({
  issueDate: Yup.string()
    .test(
      'Should not be before creation date',
      local.issuanceDateCannotBeBeforeCreationDate,
      function (this: any, value: string) {
