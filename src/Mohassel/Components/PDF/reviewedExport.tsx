                ...
                result.map((application, i) => {
                  return (
                    <View key={i} style={styles.row}>
                      ...
                      <Text style={{ width: '20%' }}>
                        {application.application.customer.customerName}
                      </Text>
                      <Text style={{ width: '10%' }}>
                        {getAge(application.application.customer.birthDate)}
                      </Text>
                      {/* <Text style={{ width: '10%' }}>{application.Application.customer.customerInfo.gender}</Text> */}
                      <Text style={{ width: '30%' }}>
                        {application.application.customer.nationalId}
                      </Text>
                      <Text style={{ width: '20%' }}>
                        {application.application.principal}
                      </Text>
                      <Text style={{ width: '20%' }}>
                        {application.application.product.noOfInstallments}
                      </Text>
                    </View>
                  )
                })}
