...
export class MapContainer extends Component<Props, State> {
  ...
  componentDidMount() {
    ...
    const input = document.getElementById('autocomplete') as HTMLInputElement // To disable any eslint 'google not defined' errors
    /* global google */ const autocomplete = new window.google.maps.places.Autocomplete(
      input,
      options
    )
      ...
      <Modal show={this.props.show} onHide={this.props.handleClose} size="lg">
        ...
        <Modal.Body style={{ minHeight: 500, padding: '0px 0px 20px 0px' }}>
          <div
            ...
            style={{ display: 'block', margin: '20px 20px 0px 20px' }}
          >
            <input
              type="text"
              id="autocomplete"
        ...
        <Modal.Footer style={{ zIndex: 3 }}>
          ...
          <Button
            variant="primary"
            onClick={() =>
              this.props.save({ lat: this.state.lat, lng: this.state.lng })
            }
          >
            {local.save}
          </Button>
